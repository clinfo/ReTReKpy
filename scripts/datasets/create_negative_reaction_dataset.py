""" The ``scripts.datasets`` directory ``create_negative_reaction_dataset`` script. """
...
def get_parser():
    ...
    arg_parser.add_argument(
        ...
        required=True,
        type=str,
        help="Path of the additional information dataset file."
    )
    ...
    arg_parser.add_argument(
        ...
        required=True,
        type=str,
        help="Path of the reaction template dataset file."
    )
...
additional_information_df = pandas.read_pickle(args.input_ai_file_path)[
    ["publication_year", "document_id", "reaction_smiles", "reaction_yield"]
]
reaction_template_df = pandas.read_csv(args.input_rt_file_path, low_memory=False)[[
...
for _, row in tqdm(
    reaction_template_df.iterrows(),
    total=len(reaction_template_df.index),
    ascii=True,
    ncols=120,
    ...
    if len([x for x in all_percentage_values if x is not None and 0 <= x <= 100]) > 0:
        reaction_yield_percentage_values.append(
            numpy.array([x for x in all_percentage_values if x is not None and 0 <= x <= 100]).mean()
        )
...
def generate_virtual_negative_examples(df_row):
    ...
    for reactant in df_row["new_reaction_smiles"].split(">>")[0].split("."):
        ...
        SanitizeMol(reactant_mol)

        reactant_mols.append(reactant_mol)

    rt_virtual_negative_products = set()

    for forward_template in sample(unique_reaction_templates, len(unique_reaction_templates)):
        if df_row["forward_template"] != forward_template and \
                len(forward_template.split(">>")[0].split(".")) == len(reactant_mols):
            try:
                reaction = ReactionFromSmarts(forward_template)

                SanitizeRxn(reaction)
                ...
                for reaction_outcome in reaction_outcomes:
                    if len(reaction_outcome) == 1:
                        try:
                            SanitizeMol(reaction_outcome[0])

                            negative_product_smiles = MolToSmiles(reaction_outcome[0], canonical=True)
        ...
        if len(rt_virtual_negative_products) >= args.rt_virtual_negatives:
            if len(rt_virtual_negative_products) > args.rt_virtual_negatives:
                rt_virtual_negative_products = sample(
    ...
    for reaction_product in sample(unique_reaction_products, len(unique_reaction_products)):
    ...
    reactant_side = ".".join([MolToSmiles(reactant_mol, canonical=True) for reactant_mol in reactant_mols])
...
with multiprocessing.Pool(args.num_cores) as process_pool:
    processing_results = [processed_entry for processed_entry in tqdm(
        iterable=process_pool.imap(generate_virtual_negative_examples, [
            row
            for _, row in reaction_template_df.iterrows()
        ]),
        total=len(reaction_template_df.index),
        ascii=True,
        ncols=120,
...
ptnvn_in_scope_df = pandas.concat([
    ptn_in_scope_df,
    pandas.DataFrame({
        "reaction_smiles": rt_virtual_negatives,
        "main_product": [reaction_smiles.split(">>")[1] for reaction_smiles in rt_virtual_negatives],
