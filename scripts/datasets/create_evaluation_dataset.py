""" The ``scripts.datasets`` directory ``create_evaluation_dataset`` script. """
...
def get_parser():
    ...
    arg_parser.add_argument(
        ...
        required=True,
        type=str,
        help="Path of any raw ChEMBL .txt file."
    )
    ...
    arg_parser.add_argument(
        ...
        type=int,
        default=10,
        help="The minimum atoms cutoff value."
    )
    ...
    arg_parser.add_argument(
        ...
        type=int,
        default=50,
        help="The maximum atoms cutoff value."
    )

    arg_parser.add_argument(
        "-fp_cutoff",
        "--fp_similarity_cutoff",
        required=False,
        type=float,
        default=0.2,
...
def standardize_chembl_entry(
    ...
    try:
        ...
        if mol is None:
            ...
            smiles_string = MolToSmiles(mol, canonical=True)
...
def run_butina_clustering(
        mol_fingerprints: List[ExplicitBitVect],
        cutoff_value: float
) -> Tuple[Tuple[int, ...]]:
    ...
    return Butina.ClusterData(clustering_distances, len(mol_fingerprints), cutoff_value, isDistData=True)


def sample_representative_compounds(
        compound_clusters: List[List[int]],
        num_samples: int
) -> List[int]:
    """ Sample an appropriate amount of representative compounds from each cluster. """

    if num_samples >= reduce(lambda count, k: count + len(k), compound_clusters, 0):
        return list(chain.from_iterable(compound_clusters))

    else:
        sampled_representative_compounds = []
        samples_per_cluster = ceil(num_samples / len(compound_clusters))

        for compound_cluster_ind, compound_cluster in enumerate(compound_clusters):
            if samples_per_cluster <= len(compound_cluster):
                sampled_representative_compounds.extend(sample(compound_cluster, samples_per_cluster))

            else:
                sampled_representative_compounds.extend(sample(compound_cluster, len(compound_cluster)))

            if len(compound_clusters) - (compound_cluster_ind + 1) != 0:
                samples_per_cluster = ceil(
                    (num_samples - len(sampled_representative_compounds)) /
                    (len(compound_clusters) - (compound_cluster_ind + 1))
                )

                if samples_per_cluster <= 0:
                    samples_per_cluster = 1

        return sample(sampled_representative_compounds, num_samples)
...
if __name__ == "__main__":
    ...
    chembl_df = pandas.read_csv(args.input_file_path, sep="\t", header=0)
    ...
    with multiprocessing.Pool(args.num_cores) as process_pool:
        processed_chembl_entries = [
            processed_entry
            for processed_entry in tqdm(
                process_pool.imap(
                    partial(standardize_chembl_entry, salts_file_path=args.salts_file_path),
                    chembl_df.values
                ),
    ...
    for partition_index in tqdm(
        range(0, int(len(processed_chembl_entries) / partition_size) + 1),
        ascii=True,
        ncols=120,
        desc="(2/3) Running the partial Butina clustering algorithm"
    ):
        if (partition_index + 1) * partition_size < len(processed_chembl_entries):
            partition_fingerprints = [
                GetMorganFingerprintAsBitVect(entry, radius=2, nBits=1024)
                for entry in processed_chembl_entries[partition_index * partition_size:(partition_index + 1) * partition_size]
            ]

        else:
            partition_fingerprints = [
                GetMorganFingerprintAsBitVect(entry, radius=2, nBits=1024)
                for entry in processed_chembl_entries[partition_index * partition_size:len(processed_chembl_entries)]
            ]
    ...
    small_evaluation_dataset.extend(sample(outlier_compound_indices, number_of_outliers_small))
    ...
    large_evaluation_dataset.extend(sample(outlier_compound_indices, number_of_outliers_large))
    ...
    print(
        "Number of large evaluation dataset entries and the representative to outlier ratio: "
    ...
    for small_evaluation_dataset_index in small_evaluation_dataset:
        small_evaluation_dataset_smiles.append(
            MolToSmiles(processed_chembl_entries[small_evaluation_dataset_index], canonical=True)
        )

    for large_evaluation_dataset_index in large_evaluation_dataset:
        large_evaluation_dataset_smiles.append(
            MolToSmiles(processed_chembl_entries[large_evaluation_dataset_index], canonical=True)
        )

    shuffle(small_evaluation_dataset_smiles)
    shuffle(large_evaluation_dataset_smiles)

    pandas.DataFrame({"canonical_smiles": small_evaluation_dataset_smiles}, dtype=str).to_csv(
        os.path.join(global_evaluation_folder_path, "small_evaluation_dataset.csv"), index=False
    )

    pandas.DataFrame({"canonical_smiles": large_evaluation_dataset_smiles}, dtype=str).to_csv(
