""" The ``scripts.datasets`` directory ``create_additional_information_dataset`` script. """
...
def get_parser():
    ...
    arg_parser.add_argument(
        ...
        required=True,
        type=str,
        help="Path of the USPTO Grants or Applications folder."
    )
    ...
    for reaction_product_ind, reaction_product in enumerate(reaction_entry.find(f"{cml_prefix}productList")):
        ...
        for product_identifier in reaction_product.findall(f"{cml_prefix}identifier"):
            if "smiles" in product_identifier.attrib["dictRef"].lower():
                try:
                    product_mol = MolFromSmiles(product_identifier.attrib["value"])

                    SanitizeMol(product_mol)

                    product_smiles = MolToSmiles(product_mol, canonical=True)
...
if __name__ == "__main__":
    ...
    for directory_name in os.listdir(args.input_folder_path):
        ...
        for file_name in tqdm(
            iterable=os.listdir(os.path.join(args.input_folder_path, directory_name)),
            total=len(os.listdir(os.path.join(args.input_folder_path, directory_name))),
            ascii=True,
            ncols=120,
            desc=f"Processing the '{directory_name}' folder"
        ):
        ...
        pandas.DataFrame(
            ...
            columns=[
                "publication_year", "cml_file", "document_id", "heading_text", "paragraph_num", "paragraph_text",
