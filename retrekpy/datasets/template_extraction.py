""" The ``retrekpy.datasets`` package ``template_extraction`` module. """
...
from .configuration import TemplateExtractionConfiguration

from ..utilities.atom_mapping_utilities import AtomMappingUtilities
...
class TemplateExtraction:
    """ The class containing the implementation of the automatic extraction of reaction rule templates. """

    def __init__(
            self,
            config_params: TemplateExtractionConfiguration
    ) -> None:
    ...
    def clean_entry(
        ...
        Output:
            (Optional[str]): A cleaned reaction SMILES string of the chemical reaction.
        """

        # Extract necessary reaction roles from the reaction SMILES string.
        reactant_smiles, _, product_smiles = FormatConversionUtilities.rxn_smiles_to_rxn_roles(rxn_smiles, verbose=verbose)

        reactant_side = ".".join(reactant_smiles)
        product_side = ".".join(product_smiles)

        # Remove the user-defined salts.
        reactant_side = StructureUtilities.remove_salts(
            reactant_side,
            salts_file_path=self.salts_file_path,
            verbose=verbose
        )

        product_side = StructureUtilities.remove_salts(
            product_side,
            salts_file_path=self.salts_file_path,
            verbose=verbose
        )
    ...
    def atom_map_entry(
        ...
        return AtomMappingUtilities.atom_map_reaction(
            rxn_smiles,
    ...
    def extract_reaction_template_from_entry(
            self,
        ...
        return AtomMappingUtilities.extract_reaction_template(mapped_rxn_smiles, verbose=verbose)

    def _collect_result(
            self,
            func_result: Optional[Any]
    ) -> Optional[Any]:
        """ Helper function to fetch the result of the asynchronously called function. """

        return func_result

    def _abortable_worker(
            self,
            func: Callable,
            *args,
            **kwargs
        ...
        thread_pool_process = dummy.Pool(1)
    ...
    def _clean_reaction_dataset(
            self,
            reaction_dataset: pandas.DataFrame,
        ...
        Input:
            reaction_dataset (pandas.DataFrame): A Pandas DataFrame of the reaction dataset that needs to be cleaned.
            rxn_smiles_column (str): The name of the column containing the reaction SMILES string of dataset entries.
            cleaned_smiles_column (str): The name of the new column containing the cleaned reaction SMILES string.

        Output:
            (pandas.DataFrame): A Pandas DataFrame containing the cleaned reaction dataset.
        """
        ...
        # Clean all reaction SMILES strings.
        if self.use_multiprocessing:
            process_pool = multiprocessing.Pool(multiprocessing.cpu_count())

            reaction_dataset[cleaned_smiles_column] = tqdm(
                process_pool.imap(self.clean_entry, reaction_dataset[rxn_smiles_column].values),
                total=len(reaction_dataset[rxn_smiles_column].values),
                ascii=True,
            ...
            reaction_dataset[cleaned_smiles_column] = [
                self.clean_entry(rxn_smiles)
                for rxn_smiles in tqdm(
                    reaction_dataset[rxn_smiles_column].values,
                    ascii=True,
                    desc="Cleaning reaction SMILES (Single Core)"
                )
            ]

        # Drop all reactions with no reactant or product side and duplicate reaction SMILES strings.
    ...
    def _atom_map_reaction_dataset(
            self,
            reaction_dataset: pandas.DataFrame,
            rxn_smiles_column: str,
        ...
        Input:
            reaction_dataset (pandas.DataFrame): A Pandas DataFrame of the reaction dataset that needs to be cleaned.
            rxn_smiles_column (str): The name of the column containing the reaction SMILES string of dataset entries.
            mapped_smiles_column (str): The name of the new column containing the mapped reaction SMILES strings.

        Output:
            (pandas.DataFrame): A Pandas DataFrame containing the mapped reaction dataset.
        """
        ...
        if self.use_multiprocessing:
            ...
            reaction_dataset[mapped_smiles_column] = tqdm(
                process_pool.imap(self.atom_map_entry, reaction_dataset[rxn_smiles_column].values),
                total=len(reaction_dataset[rxn_smiles_column].values),
                ascii=True,
            ...
            reaction_dataset[mapped_smiles_column] = [
                self.atom_map_entry(rxn_smiles)
                for rxn_smiles in tqdm(
                    reaction_dataset[rxn_smiles_column].values,
                    ascii=True,
    ...
    def _extract_reaction_templates_from_dataset(
            self,
            reaction_dataset: pandas.DataFrame,
            rxn_smiles_column: str,
        ...
        """
        A worker function for the extraction of reaction templates from the reaction SMILES entries using RDChiral.

        Input:
            reaction_dataset (pandas.DataFrame): A Pandas DataFrame of the reaction dataset that needs to be cleaned.
            rxn_smiles_column (str): The name of the column containing the reaction SMILES string of dataset entries.
            rxn_template_smarts_column (str): The new column name containing the reaction template SMARTS strings.

        Output:
            (pandas.DataFrame): A Pandas DataFrame containing the extracted reaction templates.
        """

        reaction_templates_async = []

        # Split the input Pandas DataFrame based on the length threshold for improved performance.
        short_subset = reaction_dataset.loc[
            reaction_dataset[rxn_smiles_column].str.len() <= self.extract_template_threshold
        ]

        long_subset = reaction_dataset.loc[
            reaction_dataset[rxn_smiles_column].str.len() > self.extract_template_threshold
        ]
        ...
        if self.use_multiprocessing:
            ...
            short_subset[rxn_template_smarts_column] = tqdm(
                process_pool.imap(self.extract_reaction_template_from_entry, short_subset[rxn_smiles_column].values),
                total=len(short_subset[rxn_smiles_column].values),
                ascii=True,
            ...
            short_subset[rxn_template_smarts_column] = [
                self.extract_reaction_template_from_entry(rxn_smiles)
                for rxn_smiles in tqdm(
        ...
        # Note: Using the same approach for the single core processing because of the necessary process timeout.
        if self.use_multiprocessing:
            process_pool = multiprocessing.Pool(multiprocessing.cpu_count())
            tqdm_desc = "Extracting long reaction templates (Multiple Cores)"

        else:
            process_pool = multiprocessing.Pool(1, maxtasksperchild=1)
            tqdm_desc = "Extracting long reaction templates (Single Core)"

        for rxn_smiles in tqdm(
            long_subset[rxn_smiles_column].values,
            ascii=True,
            desc=tqdm_desc
        ):
            abortable_func = partial(
                self._abortable_worker,
        ...
        return pandas.concat([short_subset, long_subset])

    def _filter_reaction_templates(
            self,
            all_reaction_templates: List[str],
            template_occurrence_threshold: int = 50
    ) -> List[str]:
        """
        ...
        # Count the occurrences of all reaction templates.
        all_reaction_templates = [
            FormatConversionUtilities.rxn_smarts_to_rxn_smiles(reaction_template)
            for reaction_template in all_reaction_templates
        ]
        ...
        # Consider only the reaction templates which occurred at least 'template_occurrence_threshold' number of times.
        filtered_templates = [
            FormatConversionUtilities.rxn_smiles_to_rxn_smarts(template_item[0])
            for template_item in sorted(template_counter.items(), key=lambda item: item[1], reverse=True)
            if template_item[1] >= template_occurrence_threshold
        ]

        return filtered_templates

    def _prepare_kgcn_templates(
            self,
            extended_dataset: pandas.DataFrame,
            template_occurrence_threshold: int = 50
    ) -> pandas.DataFrame:
        """
        ...
        Input:
            extended_dataset (pandas.DataFrame): The extended dataset dataframe.
            template_occurrence_threshold (int): A threshold value for the occurrence of a single reaction template.

        Output:
            (pandas.DataFrame): All reaction templates which occurred at least the specified amount of times.
        """

        # Since kGCN considers only one product, take the first product as the target and standardize it.
        extended_dataset["products"] = [
            p.split(">>")[1].split(".")[0]
            for p in extended_dataset[self.mapped_smiles_column].values
        ]

        extended_dataset["is_valid"] = [
            FormatConversionUtilities.smiles_to_mol(p) is not None
            for p in extended_dataset["products"].values
        ]
        ...
        extended_dataset["products"] = [
            FormatConversionUtilities.mol_to_smarts(FormatConversionUtilities.smiles_to_mol(p))
            for p in extended_dataset["products"].values
        ]
    ...
    def extract_reaction_templates(
            self,
            save_extended_dataset: bool = True,
            verbose: bool = False
    ) -> List[str]:
        """ A user-friendly version of the automatic template extraction function. """

        # Step 1: Read the input dataset that needs to be pre-processed.
        reaction_dataset = DataUtilities.read_dataset(
        ...
        reaction_dataset = self._clean_reaction_dataset(
        ...
        reaction_dataset = self._atom_map_reaction_dataset(
            reaction_dataset=reaction_dataset,
        ...
        reaction_dataset = self._extract_reaction_templates_from_dataset(
            reaction_dataset=reaction_dataset,
        ...
        # Step 5: Filter the extracted reaction templates keeping the ones which occurred the specified number of times.
        final_templates = self._filter_reaction_templates(
            reaction_dataset[self.rxn_template_smarts_column].values.tolist(),
        ...
        kgcn_templates_dataset = self._prepare_kgcn_templates(
            reaction_dataset,
            template_occurrence_threshold=self.template_occurrence_threshold
        )
        ...
        if save_extended_dataset:
            if verbose:
                print(
                    "Saving the reaction dataset as '{}extended_dataset.csv'...".format(self.output_folder_path),
                    end="",
                    flush=True
                )
        ...
        if verbose:
            print(
                "Saving the filtered reaction templates as '{}_x{}{}'...".format(
        ...
        DataUtilities.save_dataset(
            pandas.DataFrame({"smarts": final_templates}),
            output_folder_path=self.output_folder_path,
        ...
        if verbose:
            print(
                "Saving the kGCN input file as 'kgcn_x{}.csv'...".format(self.template_occurrence_threshold),
                end="",
                flush=True
            )

        DataUtilities.save_dataset(
            kgcn_templates_dataset,
            output_folder_path=self.output_folder_path,
